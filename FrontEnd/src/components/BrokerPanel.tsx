...
export const BrokerPanel: React.FC<BrokerPanelProps> = ({ signalData }) => {
  ...
  const lastExec = signalData?.model_info?.last_exec ?? null;
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 overflow-y-auto px-2 pb-2 space-y-2">
        ...
        <ExecEventCard lastExec={lastExec} />
...
const DefaultsForm: React.FC<{
  ...
  const [btcQtyText, setBtcQtyText] = useState(String(config.default_btc_qty));
  const [ethQtyText, setEthQtyText] = useState(String(config.default_eth_qty));
  const [busy, setBusy] = useState(false);
  const [feedback, setFeedback] = useState<{ kind: 'success' | 'error'; text: string } | null>(null);

  // Keep form in sync with server-truth nudges (WS pushes etc.)
  ...
  useEffect(() => { setBtcQtyText(String(config.default_btc_qty)); }, [config.default_btc_qty]);
  useEffect(() => { setEthQtyText(String(config.default_eth_qty)); }, [config.default_eth_qty]);

  const qty = Number(qtyText);
  const btcQty = Number(btcQtyText);
  const ethQty = Number(ethQtyText);
  const symbolValid = SYMBOL_RE.test(symbol);
  const qtyValid = Number.isFinite(qty) && qty > 0;
  const btcQtyValid = Number.isFinite(btcQty) && btcQty > 0;
  const ethQtyValid = Number.isFinite(ethQty) && ethQty > 0;
  const dirty =
    symbol !== config.default_symbol ||
    qty !== config.default_qty ||
    btcQty !== config.default_btc_qty ||
    ethQty !== config.default_eth_qty;
  const canSave = symbolValid && qtyValid && btcQtyValid && ethQtyValid && dirty && !busy;
  ...
  const onSave = async () => {
    ...
    const result = await update({
      ...
      default_qty: qty,
      default_btc_qty: btcQty,
      default_eth_qty: ethQty,
    });
    setBusy(false);
    if (result.ok) {
  ...
  return (
    ...
    >
      ...
      <label className="block mb-1">
        <span className="text-[10px]" style={{ color: THEME_COLORS.TEXT_SECONDARY }}>Symbol (manual orders)</span>
        <input
          type="text"
          value={symbol}
      ...
      <label className="block mb-1">
        <span className="text-[10px]" style={{ color: THEME_COLORS.TEXT_SECONDARY }}>Qty (manual orders)</span>
        <input
          type="number"
          value={qtyText}
          step="0.0001"
      ...
      <div className="mt-1 mb-0.5 text-[10px] font-semibold" style={{ color: THEME_COLORS.TEXT_SECONDARY }}>
        Auto-execute legs
      </div>
      <label className="block mb-1">
        <span className="text-[10px]" style={{ color: THEME_COLORS.TEXT_SECONDARY }}>BTC qty</span>
        <input
          type="number"
          value={btcQtyText}
          step="0.0001"
          min="0"
          onChange={e => setBtcQtyText(e.target.value)}
          style={inputStyle}
        />
        {!btcQtyValid && (
          <span className="text-[10px]" style={{ color: THEME_COLORS.NEGATIVE }}>Must be positive</span>
        )}
      </label>
      <label className="block mb-1">
        <span className="text-[10px]" style={{ color: THEME_COLORS.TEXT_SECONDARY }}>ETH qty</span>
        <input
          type="number"
          value={ethQtyText}
          step="0.001"
          min="0"
          onChange={e => setEthQtyText(e.target.value)}
          style={inputStyle}
        />
        {!ethQtyValid && (
          <span className="text-[10px]" style={{ color: THEME_COLORS.NEGATIVE }}>Must be positive</span>
...
const ExecEventCard: React.FC<{ lastExec: BrokerExecEvent | null }> = ({ lastExec }) => {
  if (!lastExec) return null;

  const time = new Date(lastExec.timestamp).toLocaleTimeString();
  const isReconciled = lastExec.reconciled === true;
  const headerColor = lastExec.all_ok ? THEME_COLORS.POSITIVE : THEME_COLORS.NEGATIVE;

  const statusColor = (leg: BrokerExecEvent['legs'][number]) => {
    if (leg.status === 'REJECTED' || leg.status === 'ERROR') return THEME_COLORS.NEGATIVE;
    if (leg.status === 'PARTIALLY_FILLED') return THEME_COLORS.YELLOW;
    // Unreconciled NEW: show muted — we don't know if it filled yet
    if (leg.status === 'NEW' && !leg.reconciled) return THEME_COLORS.TEXT_SECONDARY;
    return THEME_COLORS.POSITIVE; // FILLED, or NEW confirmed, etc.
  };

  const statusLabel = (leg: BrokerExecEvent['legs'][number]) => {
    const id = leg.order_id ? ` #${leg.order_id.slice(-6)}` : '';
    if (leg.status === 'FILLED' || (leg.status === 'NEW' && leg.reconciled && leg.filled_qty > 0)) {
      // Confirmed fill — show qty and price
      const qty = leg.filled_qty > 0 ? leg.filled_qty : leg.qty;
      const price = leg.avg_price > 0 ? ` @ $${leg.avg_price.toFixed(2)}` : '';
      return `FILLED ${qty}${price}${id}`;
    }
    if (leg.status === 'NEW') return `SUBMITTED${id}`;
    return `${leg.status}${id}`;
  };
  ...
  return (
    <section
      style={{
        backgroundColor: THEME_COLORS.CARD_BG_LIGHT,
        border: `1px solid ${lastExec.all_ok ? THEME_COLORS.POSITIVE : THEME_COLORS.NEGATIVE}44`,
        borderRadius: 6,
        padding: '6px 8px',
      }}
    >
      <div className="flex items-center justify-between mb-1">
        <span className="text-[10px] font-bold tracking-widest" style={{ color: headerColor }}>
          LAST AUTO-EXEC
        </span>
        <div className="flex items-center gap-2">
          {!isReconciled && (
            <span className="text-[9px]" style={{ color: THEME_COLORS.TEXT_SECONDARY }}>
              confirming fills…
            </span>
          )}
          <span className="text-[10px]" style={{ color: THEME_COLORS.TEXT_SECONDARY }}>{time}</span>
        </div>
      </div>
      <div className="text-[11px] font-semibold mb-1.5" style={{ color: THEME_COLORS.TEXT_PRIMARY }}>
        {lastExec.transition}
        <span
        ...
        >
          {lastExec.all_ok ? (isReconciled ? 'FILLED' : 'OK') : 'FAILED'}
        </span>
      </div>
      <div className="space-y-0.5">
        {lastExec.legs.map((leg, i) => (
          <div key={i} className="flex items-center justify-between text-[10px]">
            <span style={{ color: THEME_COLORS.TEXT_SECONDARY }}>
              {leg.symbol}
              {' '}
              <span style={{ color: leg.side === 'BUY' ? THEME_COLORS.POSITIVE : THEME_COLORS.NEGATIVE }}>
                {leg.side}
              </span>
              {' '}{leg.qty}
              {leg.reduce_only && (
                <span style={{ color: THEME_COLORS.TEXT_SECONDARY }}> [exit]</span>
              )}
            </span>
            <span style={{ color: statusColor(leg) }}>
              {statusLabel(leg)}
            </span>
          </div>
        ))}
      </div>
      {!lastExec.all_ok && lastExec.legs.some(l => l.error) && (
        <div className="mt-1 text-[9px]" style={{ color: THEME_COLORS.NEGATIVE }}>
          {lastExec.legs.filter(l => l.error).map((l, i) => (
            <div key={i}>{l.symbol}: {l.error}</div>
          ))}
        </div>
      )}
    </section>
